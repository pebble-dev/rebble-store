...
<template>
  ...
  <div
    ...
    v-bind:class="(this.$store.state.userParameters.inApp) ? 'app-title-bar-cont sticky-top': 'app-title-bar-cont'"
  >
    <div class="card subsection-inverse card-inverse text-left p-3 app-title-bar">
      <img
        class="app-icon"
        v-if="app.icon_image != null && app.icon_image['48x48'] != ''"
        v-bind:src="app.icon_image['48x48']"
      />
      ...
      <div class="app-button-container float-right">
        <button
          type="button"
          v-bind:class="heartClass"
          v-on:click="toggle_heart_button_state"
          ref="heartsButton"
        >
          <svg class="svg-icon icon-thumbs-up" width="25px" height="25px" viewBox="0 0 25 25">
            <use xlink:href="#iconThumbsUp" />
          </svg>
          {{ hearts }}
        </button>
    ...
    <div
      class="card subsection-extra card-inverse text-left p-2"
      v-if=" app.companions != undefined && (app.companions.ios != null || app.companions.android != null) && app.type != 'watchface'"
    >
      ...
      <div class="ml-auto">
        <a v-if="app.companions.ios" v-bind:href="app.companions.ios.url" target="_blank">
          <svg class="app-icon" width="22px" height="22px">
            <use xlink:href="#iconApple" />
          </svg>
        </a>
        <h2 v-if="app.companions.ios && app.companions.android">+</h2>
...
export default {
  ...
  methods: {
    get_user_data: function (id) {
      if (
        this.$store.state.secure.accessToken !== "" &&
        this.$store.state.secure.accessToken != null
      ) {
        this.$http
          .get(this.$store.state.config.devPortalBackendUrl + "/users/me", {
            headers: {
              Authorization: "Bearer " + this.$store.state.secure.accessToken,
            },
          })
          .then(
            (response) => {
    ...
    change_heart: function (operation) {
      this.$http
        .post(
          this.$store.state.config.devPortalBackendUrl +
            "/applications/" +
            this.app.id +
            "/" +
            operation +
