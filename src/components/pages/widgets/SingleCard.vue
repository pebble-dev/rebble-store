<template>
  <div v-bind:class="imageLoaded ? 'loaded' : 'loading'">
    <router-link
      class="real-card"
      v-bind:to="'/app/' + card.id"
      v-images-loaded="loaded"
    >
      <div
        class="card"
        :class="$store.state.userParameters.hardware == 'chalk' ? 'round' : ''"
      >
        <img
          class="card-img-top"
          v-bind:src="
            card.screenshot_images[0][Object.keys(card.screenshot_images[0])[0]]
          "
          alt="App Icon"
        />
        <div class="card-block text-xs-center">
          ...
          <p class="card-text">
            <small class="text-muted">
              <svg
                class="svg-icon icon-inverted-thumbs-up"
                width="16px"
                height="16px"
                viewBox="0 0 25 25"
              >
...
export default {
  ...
  methods: {
    ...
    build_from_search: function () {
      ...
      const hardware = this.$store.state.userParameters.hardware
